...
function ReaderModal({ isOpen, onClose, currentCustomer, onSubmit }) {
  const isEdit = !!(
  ...
  useEffect(() => {
    ...
    if (currentCustomer) {
      ...
      // lấy fullname từ fullName hoặc fullname
      const fullname = currentCustomer.fullName || currentCustomer.fullname || "";
      ...
      setFormValues({
        ...
        birthDate: currentCustomer.birthDate
            ? new Date(currentCustomer.birthDate).toISOString().split("T")[0]
            : "",
        age: currentCustomer.age != null ? String(currentCustomer.age) : "",
        password: "",
        confirmPassword: "",
        is_deleted: String(!!(currentCustomer.is_deleted ?? false)),
        is_active: String(!!(currentCustomer.is_active ?? true)),
      });
    } else {
  ...
  const validateField = (name, values) => {
    ...
    // ✅ Create-only (giống EmployeeModal)
    ...
    if (!isEdit && name === "birthDate") {
      ...
      if (!isAtLeast18(birthDate)) return "Người dùng phải từ 18 tuổi trở lên.";
      return null;
    }
  ...
  const validateAll = (values) => {
    const fields = isEdit
        ...
        : ["fullname", "username", "email", "password", "confirmPassword", "birthDate", "age"];
