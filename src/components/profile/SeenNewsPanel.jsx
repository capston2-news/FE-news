// src/components/home/SeenNewsPanel.jsx
...
import {
  ...
  getHistoryArticleOfUser, // <-- đảm bảo bạn đã export hàm này từ ArticleService
...
const normalizeApiArticle = (a) => {
  ...
  return {
    ...
    // optional: thời điểm xem (nếu API có)
    viewed_at: a?.viewed_at?.$date || a?.viewed_at || "",
  };
};

// UI card (giống SavedNewsPanel)
...
export default function SeenNewsPanel() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const res = await getHistoryArticleOfUser(); // ✅ API lịch sử xem
        const raw =
          Array.isArray(res)
            ? res
        ...
        // init map bookmark từ API (is_bookmarked)
        const next = {};
        for (const a of raw) {
          const oid = oidOf(a);
          if (!oid) continue;
          next[oid] = !!(a?.is_bookmarked ?? false);
        }
        setBookmarkByOid(next);
      } catch {
        setError("Không tải được danh sách tin đã xem.");
        setApiList([]);
        setBookmarkByOid({});
      } finally {
        setLoading(false);
  ...
  const toggleBookmark = async (oid) => {
    ...
    const isMarked = !!bookmarkByOid[oid];

    // optimistic
    setBookmarkByOid((m) => ({ ...m, [oid]: !isMarked }));

    const toastId = toast.loading(isMarked ? "Đang bỏ lưu..." : "Đang lưu...");
    ...
    try {
      ...
      // revert
      setBookmarkByOid((m) => ({ ...m, [oid]: isMarked }));
      toast.error(isMarked ? "Không thể bỏ lưu. Thử lại nhé!" : "Không thể lưu. Thử lại nhé!", {
        id: toastId,
      });
    }
  };
    ...
    return (
      <section className="max-w-5xl mx-auto px-4 py-6">
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          <h2 className="text-base font-semibold text-slate-900">Tin đã xem</h2>
          <p className="mt-1 text-sm text-slate-600">
            Bạn cần đăng nhập để xem lịch sử tin đã xem.
          </p>
          <button
            type="button"
  ...
  return (
    <section className="max-w-5xl mx-auto px-4 py-6">
      ...
      <div className="flex items-center justify-between mb-2">
        <div>
          <h3 className="text-xl font-bold text-slate-900">Tin đã xem</h3>
        </div>
      ...
      {!loading && !error && list.length === 0 && (
        <div className="rounded-2xl border border-slate-200 bg-white p-8 text-center">
          ...
          <p className="text-sm text-slate-600">
            Chưa có tin đã xem. Khi bạn mở bài viết, hệ thống sẽ lưu vào đây.
          </p>
        </div>
      )}
      ...
      <div>
        {currentList.map((item, idx) => {
          ...
          return (
            <div
              key={key}
              className={"py-3 " + (isLast ? "" : "border-b border-slate-300/60")}
            >
              <FeedArticleCard
                article={item}
                articleId={articleId}
                ...
                onToggleBookmark={() => toggleBookmark(oid)}
              />
            </div>
          );
        })}
      ...
      {totalPages > 1 && (
        <div className="mt-6 flex items-center justify-center gap-3 text-sm text-slate-700 select-none">
          <button
            ...
            className="w-9 cursor-pointer h-9 rounded-xl border border-slate-200 bg-white disabled:opacity-40
            ...
            {pages.map((p) => {
              ...
              return (
                <button
                  ...
                  onClick={() => goToPage(p)}
                  className={
                    "w-9 h-9 rounded-xl cursor-pointer border transition " +
                    (active
